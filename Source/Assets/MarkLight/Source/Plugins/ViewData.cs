        ...
        public static void GenerateViews()
        {
            ...
            foreach (var viewType in viewPresenter.ViewTypeDataList)
            {
                ...
                foreach (var dependencyName in viewType.DependencyNames)
                {
                    ...
                    if (dependency == null)
                    {
                        Debug.LogError(String.Format("[MarkLight] {0}: View contains the child view \"{1}\" that could not be found.", viewType.ViewTypeName, dependencyName));
                        continue;
                    }
        ...
        public static View CreateView(string viewName, View layoutParent, View parent, ValueConverterContext context = null, string theme = "", string id = "", string style = "", IEnumerable<XElement> contentXuml = null)
        {
            ...
            foreach (var referenceField in viewTypeData.ReferenceFields)
            {
                ...
                if (referencedView != null)
                {
                    ...
                    {
                        Debug.LogError(String.Format("[MarkLight] {0}: Unable to set view reference field \"{1}\". Referenced type \"{2}\" cannot be converted to target type \"{3}\".", viewTypeData.ViewTypeName, referenceField, referencedViewType.FullName, referenceFieldInfo.FieldType.FullName));
                        continue;
                    }
